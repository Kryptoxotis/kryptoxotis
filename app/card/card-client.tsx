...
function buildVcf(p: CardProps) {
  return `BEGIN:VCARD
VERSION:3.0
FN:${p.cardName}
ORG:Kryptoxotis
TITLE:${p.cardTitle}
TEL;TYPE=CELL:+1${p.phone.replace(/\D/g, "")}
EMAIL:${p.email}
URL:${p.siteUrl}
ADR;TYPE=WORK:;;${p.address};;;US
END:VCARD`
}

export default function CardClient(props: CardProps) {
  ...
  const saveContact = () => {
    const blob = new Blob([buildVcf(props)], { type: "text/vcard" })
    const url = URL.createObjectURL(blob)
    const a = document.createElement("a")
    a.href = url
    a.download = `${props.cardName.replace(/\s+/g, "_")}_Kryptoxotis.vcf`
    a.click()
  ...
  const contacts = [
    { icon: Phone, label: phoneFormatted, href: `tel:+1${phoneDigits}`, color: "from-emerald-400 to-emerald-600" },
    { icon: Mail, label: props.email, href: `mailto:${props.email}`, color: "from-emerald-400 to-teal-500" },
    { icon: Globe, label: props.siteUrl.replace(/^https?:\/\//, ""), href: props.siteUrl, color: "from-teal-400 to-cyan-500" },
    { icon: MapPin, label: props.address, href: `https://maps.google.com/?q=${encodeURIComponent(props.address)}`, color: "from-cyan-400 to-emerald-500" },
  ]

  const socials = [
    { icon: Instagram, label: "Instagram", href: props.instagramUrl, hoverColor: "hover:text-pink-400 hover:shadow-pink-500/30" },
    { icon: Youtube, label: "YouTube", href: props.youtubeUrl, hoverColor: "hover:text-red-400 hover:shadow-red-500/30" },
    { icon: Twitter, label: "X / Twitter", href: props.twitterUrl, hoverColor: "hover:text-sky-400 hover:shadow-sky-500/30" },
  ]
        ...
        >
          ...
          <div className="relative z-10 bg-zinc-950/95 backdrop-blur-xl rounded-2xl p-8 sm:p-10">
            ...
            <div className={`text-center mb-8 transition-all duration-700 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`} style={{ transitionDelay: "0.5s" }}>
              ...
              <p className="text-white text-lg font-medium">{props.cardName}</p>
              <p className="text-emerald-400/80 text-sm tracking-widest uppercase mt-1">{props.cardTitle}</p>
            </div>
            ...
            <div className={`flex justify-center gap-3 mb-8 transition-all duration-700 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`} style={{ transitionDelay: "1.0s" }}>
              ...
              <button
                onClick={() => {
                  navigator.clipboard.writeText(props.zelleNumber)
                  setZelleCopied(true)
